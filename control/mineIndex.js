	...
	obj.control.set({
		...
		fn:function(data){
			...
			function layout(at,user){
				...
				$(".mine_top .message").unbind("click").bind("click",function(){
					window.location.hash="messageIndex/0/0";
					});
				$(".mine_top .deg").unbind("click").bind("click",function(){
					window.location.hash="selfInfo";
					});
			$(".mine_top .button.login").unbind("click").bind("click",function(){
				window.location.hash="login/mineIndex";
				});
			$(".mine_top .button.regist").unbind("click").bind("click",function(){
				window.location.hash="regist";
				});
			$(".mine_top .bottom .point").unbind("click").bind("click",function(){
				var that=this;
				if(user){
					if($(that).attr("src")){
			...
			$(".mine_bottom .point").unbind("click").bind("click",function(){
				var that=this;
				if(user){
					if($(that).attr("href")){
